...
Lista * CriarLista(int C) {
    ...
    if (L->Dados == NULL) {
        printf("ERRO: nao a memoria para armazenar o vetor de tarefas!\n");
        free(L); // Para liberar a memoria armazenada no ponteiro
        return NULL;
    }

    // printf("O vetor de dados esta armazenada no endereco %X\n", L->Dados);
